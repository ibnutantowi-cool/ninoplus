const fs = require('fs');
fs.writeFileSync('src/app/berita/[id]/page.tsx', `import { db } from "@/lib/db";
    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <div className="bg-gradient-to-br from-[#061123] via-[#0B192C] to-[#0f2442] pt-24 pb-32 relative overflow-hidden z-10">
        ...
        <div className="container mx-auto px-4">
          <Link href="/tentang" className="inline-flex items-center gap-2 text-cyan-400 font-bold hover:text-cyan-300 transition-colors mb-8">
            <ArrowLeft className="w-5 h-5" /> Kembali ke Pusat Informasi
          </Link>
          <div className="text-xs font-bold text-cyan-300 uppercase tracking-widest mb-4 bg-white/10 inline-block px-3 py-1 rounded-full border border-white/20 backdrop-blur-sm">
            {berita.category}
          </div>
      ...
      <div className="flex-grow bg-slate-50 text-slate-800 pb-20 -mt-20 relative z-20">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto bg-white rounded-3xl shadow-xl border border-slate-200 overflow-hidden">
            ...
            {berita.imageUrl && (
              <div className="w-full h-64 md:h-[500px] relative bg-slate-100">
                <img 
                  src={berita.imageUrl} 
                  alt={berita.title}
